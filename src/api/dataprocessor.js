...
function DataProcessor(response){
    ...
    for(let i=1;i<range.values.length;i++){
        var answer = [];
        for(j=1;j<range.values[i].length-2;j++){
    ...
    this.FetchPersonalData = function(email0){
      for(i=0;i<this.ALL_RESPONSES.length;i++){
    ...
    this.CountData4Graph = function(data){
      result = {
        MBP: 0,
        PMBP:0,
        NMBP:0,
    ...
    this.getPersonalData = function(useremail){
        return this.FetchPersonalData(useremail);
    }
    this.getData4Graph = function(useremail){
        var personaldata = this.FetchPersonalData(useremail);
        return this.CountData4Graph(personaldata);
    }
}
